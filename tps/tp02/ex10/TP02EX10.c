...
int compareHouseTo(Personagem *a, Personagem *b)
{
  return strcmp(a->house, b->house);
}
...
// Quick Sort - House
void quickSortRec(Personagem personagens[], int esq, int dir)
{
  int i = esq;
  int j = dir;
  Personagem pivo = personagens[(i + j) / 2];

  while (i <= j)
  {
    while ((compareHouseTo(&personagens[i], &pivo) < 0) || (compareHouseTo(&personagens[i], &pivo) == 0 && compareNameTo(&personagens[i], &pivo) < 0))
    {
      i++;
    }
    while ((compareHouseTo(&personagens[j], &pivo) > 0) || (compareHouseTo(&personagens[j], &pivo) == 0 && compareNameTo(&personagens[j], &pivo) > 0))
    {
      j--;
    }
  ...
  if (esq < j)
    quickSortRec(personagens, esq, j);
  if (i < dir)
    quickSortRec(personagens, i, dir);
}

void quickSort(Personagem personagens[], int n)
{
  quickSortRec(personagens, 0, n - 1);
}
