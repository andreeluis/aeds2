...
public class TP02EX07 {
  public static void main(String[] args) {
    ...
    // ordena por inserção
    for (int i = 1; i < tam; i++) {
      Personagem tmp = pSelecionados[i];
      int j = i - 1;

      while (j >= 0) {
        int dateComparison = tmp.getDateOfBirth().compareTo(pSelecionados[j].getDateOfBirth());
        int nameComparison = tmp.getName().compareTo(pSelecionados[j].getName());

        boolean isSameDateButEarlierName = dateComparison == 0 && nameComparison < 0;
        boolean isEarlierDate = dateComparison < 0;

        if (isSameDateButEarlierName || isEarlierDate) {
          pSelecionados[j + 1] = pSelecionados[j];
          j--;
        } else {
          break;
        }
      }
      pSelecionados[j + 1] = tmp;
    }

    for (int i = 0; i < tam; i++) {
      pSelecionados[i].imprimir();
    }
