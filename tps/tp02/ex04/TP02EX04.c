...
int main()
{
  ...
  char line[300];                  // Aumente o tamanho se necessário
  ...
  for (int i = 0; i < tam - 1; i++)
  {
    for (int j = i + 1; j < tam; j++)
    {
      ...
      {
        Personagem temp = pSelecionados[i];
  ...
  while (strcmp(nome, "FIM"))
  {
    ...
    while (inicio <= fim) {
      ...
      //printf("%s - i:%d m:%d f:%d -> %s\n", nome, inicio, meio, fim, pSelecionados[meio].name);

      if (strcmp(nome, pSelecionados[meio].name) == 0) {
        encontrado = true;
        break;
      } else if (strcmp(nome, pSelecionados[meio].name) < 0) {
        fim = meio - 1;
      } else {
        inicio = meio + 1;
      }
