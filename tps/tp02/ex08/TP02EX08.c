...
int compareEyeTo(Personagem *a, Personagem *b)
{
  return strcmp(a->eyeColour, b->eyeColour);
}
...
void shellSort(Personagem personagens[], int n)
{
  for (int gap = n / 2; gap > 0; gap /= 2)
  {
    for (int i = gap; i < n; i += 1)
    {
      ...
      for (j = i; j >= gap; j -= gap)
      {
        int eyeDiff = compareEyeTo(&personagens[j - gap], &temp);
        int nameDiff = compareNameTo(&personagens[j - gap], &temp);
        if (eyeDiff > 0 || (eyeDiff == 0 && nameDiff > 0))
