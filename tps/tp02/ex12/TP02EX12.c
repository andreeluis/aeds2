...
void bubbleSort(Personagem personagens[], int n)
{
  for (int i = 0; i < n - 1; i++)
  {
    ...
    {
      if ((compareHairTo(&personagens[j], &personagens[j + 1]) > 0) || (compareHairTo(&personagens[j], &personagens[j + 1]) == 0) && (compareNameTo(&personagens[j], &personagens[j + 1]) > 0))
      {
