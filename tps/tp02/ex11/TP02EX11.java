...
class CountingSort {
  ...
  public CountingSort(Personagem[] array, int tam) {
    this.array = array;
    this.tam = tam;

    // ordena o array por ordem alfabetica de nome
    for (int i = 0; i < tam - 1; i++) {
      int menor = i;

      for (int j = i + 1; j < tam; j++) {
        if (array[j].compareNameTo(array[menor]) < 0) {
          menor = j;
        }
      }
  ...
  public void sort() {
    Personagem[] output = new Personagem[tam];
    int[] count = new int[getMaior().getYearOfBirth() + 1];

    for (int i = 0; i < tam; i++) {
      count[array[i].getYearOfBirth()]++;
    }
    ...
    for (int i = tam - 1; i >= 0; i--) {
      output[count[array[i].getYearOfBirth()] - 1] = array[i];
      count[array[i].getYearOfBirth()]--;
    }
  ...
  public Personagem getMaior() {
    ...
    for (int i = 0; i < tam; i++) {
      if (maior.getYearOfBirth() < array[i].getYearOfBirth()) {
        maior = array[i];
      }
    }
...
public class TP02EX11 {
  public static void main(String[] args) {
    ...
    CountingSort countingSort = new CountingSort(pSelecionados, tam);
    countingSort.sort();
    countingSort.imprimir();
