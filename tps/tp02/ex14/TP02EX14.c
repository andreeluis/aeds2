...
int compareIdTo(Personagem *a, Personagem *b)
{
...
// Radix Sort - ID
long getMax(Personagem array[], int n) {
  long max = strtol(array[0].id, NULL, 10);

  for (int i = 1; i < n; i++) {
    long id = strtol(array[i].id, NULL, 10);
    if (max < id)
      max = id;
  }
...
void radcountingSort(Personagem array[], int n, int exp) {
  ...
  for (int i = 0; i < n; i++) {
    int index = (strtol(array[i].id, NULL, 10) / exp) % 10;
    count[index]++;
  }
