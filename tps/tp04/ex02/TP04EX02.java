...
class ArvoreBinariaNome {
  ...
  private NodeNome inserir(Personagem p, NodeNome node) {
    if (node == null)
      node = new NodeNome(p);
...
class ArvoreBinaria {
  ...
  private void inserir(int modAno) {
    root = inserir(modAno, root);
  }

  private Node inserir(int modAno, Node node) {
    if (node == null)
      node = new Node(modAno);
    else if (modAno < node.ano)
      node.esq = inserir(modAno, node.esq);
    else if (modAno > node.ano)
      node.dir = inserir(modAno, node.dir);
  ...
  private Node inserir(Personagem p, Node node) {
    int value = p.getYearOfBirth() % 15;

    if (value < node.ano)
      node.esq = inserir(p, node.esq);
    else if (value > node.ano)
      node.dir = inserir(p, node.dir);
    else
      node.root.inserir(p);
