...
class Lista {
  ...
  public void inserir(Personagem personagem, int pos) {
    ...
    for (int i = 1; i < pos; current = current.prox, i++);
  ...
  public void inserirFim(Personagem personagem) {
    tail.prox = new Celula(personagem);
    tail = tail.prox;
  }

  public Personagem removerInicio() {
    Celula aux = this.head.prox;
    head.prox = aux.prox;
  ...
  public Personagem removerFim() {
    ...
    Celula current;
    for (current = head.prox; current.prox != tail; current = current.prox);
  ...
  public void imprimir() {
    int cont = 0;
    for (Celula i = head.prox; i != null; i = i.prox, cont++) {
      System.out.println(i.personagem.toString(cont));
    }
  }
}

public class TP03EX05 {
