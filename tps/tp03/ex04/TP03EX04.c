...
typedef struct Fila {
	Personagem *personagens[TAM + 1];	// array de ponteiro de personagem
	int pri;
  int ult;
} Fila;
...
void imprimirFila(Fila *fila) {
  for (int i = fila->pri, cont = 0; i != fila->ult; i = (i + 1) % TAM) {
    imprimirPersonagem(fila->personagens[i], cont++);
  }
}

void inserir(Fila *fila, Personagem *personagem) {
  if (((fila->ult + 1) % TAM) == fila->pri) {
    fila->pri = ((fila->pri + 1) % TAM);
  }

  fila->personagens[fila->ult] = personagem;
  fila->ult = ((fila->ult + 1) % TAM);
}

Personagem *remover(Fila *fila) {
	Personagem *aux = fila->personagens[fila->pri];
  fila->pri = ((fila->pri + 1) % TAM);
...
int mediaAnos(Fila *fila) {
  int total = 0, cont = 0;
  for (int i = fila->pri; i != fila->ult; i = (i + 1) % TAM, cont++) {
    total += getYearOfBirth(fila->personagens[i]);
  }
