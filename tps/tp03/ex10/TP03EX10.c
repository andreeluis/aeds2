...
Pilha newPilha() {
	...
	pilha.pri = pilha.ult = malloc(sizeof(Celula));
...
void imprimirPilha(Pilha *pilha) {
  int cont = 0;
  for (Celula *i = pilha->pri; i != pilha->ult; i = i->prox) {
...
Personagem *remover(Pilha *pilha) {
  Personagem *aux = pilha->pri->personagem;

  Celula *temp = pilha->pri;
  pilha->pri = pilha->pri->prox;
...
int main() {
	...
	Pilha pilhaPersonagens = newPilha();

	char id[36];
	scanf("%s", id);
	while (strcmp(id, "FIM")) {
		// Busca personagem pelo id e insere no fim da lista
		inserir(&pilhaPersonagens, buscarPersonagem(id, personagens, tot));
	...
	for (int i = 0; i < qtde; i++) {
		...
		if (strcmp(op, "I") == 0) {
			...
			printf("(R) %s\n", getName(remover(&pilhaPersonagens)));
    }
	}

  printf("[ Top ]\n");
	imprimirPilha(&pilhaPersonagens);
  printf("[ Bottom ]\n");
