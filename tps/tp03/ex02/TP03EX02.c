...
typedef struct Lista {
	Personagem **personagens;	// array de ponteiro de personagem
	int n;
} Lista;

Lista newLista(int qtde) {
	Lista lista;

	lista.personagens = malloc(qtde * sizeof(Personagem*));
	lista.n = 0;
...
void imprimirLista(Lista *lista) {
	for (int i = 0; i < lista->n; i++) {
		imprimirPersonagem(lista->personagens[i], i);
	}
}
...
void inserirInicio(Lista *lista, Personagem *personagem) {
	inserir(lista, personagem, 0);
}

void inserirFim(Lista *lista, Personagem *personagem) {
	inserir(lista, personagem, lista->n);
}
...
Personagem *remover(Lista *lista, int pos) {
	...
	Personagem *aux = lista->personagens[pos];
...
int main() {
	...
	Lista listaPersonagens = newLista(tot);
	...
	for (int i = 0; i < qtde; i++) {
		...
		if (strcmp(op, "II") == 0) {
			scanf("%s", id);
			inserirInicio(&listaPersonagens, buscarPersonagem(id, personagens, tot));
		} else if (strcmp(op, "IF") == 0) {
			scanf("%s", id);
			inserirFim(&listaPersonagens, buscarPersonagem(id, personagens, tot));
		} else if (strcmp(op, "I*") == 0) {
			scanf("%d %s", &pos, id);
			inserir(&listaPersonagens, buscarPersonagem(id, personagens, tot), pos);
		} else if (strcmp(op, "RI") == 0) {
			printf("(R) %s\n", getName(removerInicio(&listaPersonagens)));
		} else if (strcmp(op, "RF") == 0) {
			printf("(R) %s\n", getName(removerFim(&listaPersonagens)));
		} else {
			scanf("%d", &pos);
			printf("(R) %s\n", getName(remover(&listaPersonagens, pos)));
		}
	}
